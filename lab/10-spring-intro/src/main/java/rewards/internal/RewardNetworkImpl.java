...
public class RewardNetworkImpl implements RewardNetwork {
    ...
    public RewardConfirmation rewardAccountFor(Dining dining) {
        Account paidAccount = accountRepository.findByCreditCard(dining.getCreditCardNumber());
        Restaurant restaurant = restaurantRepository.findByMerchantNumber(dining.getMerchantNumber());

        MonetaryAmount monetaryAmount = restaurant.calculateBenefitFor(paidAccount, dining);
        AccountContribution accountContribution = paidAccount.makeContribution(monetaryAmount);
        accountRepository.updateBeneficiaries(paidAccount);
